...
export function ChatPanel({ chatId, chat, apiKey, onClose }: ChatPanelProps) {
  ...
  return (
    <div className="flex flex-1 flex-col border-r border-gray-200 dark:border-gray-700 last:border-r-0">
      {/* Header */}
      <div className="flex items-center justify-between border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 px-4 py-2">
        <div className="flex items-center gap-2">
          ...
          />
          {!connected && (
            <span className="rounded bg-amber-100 dark:bg-amber-800/40 text-amber-700 dark:text-amber-300 px-1.5 py-0.5 text-[10px]">
              Reconnecting...
            </span>
          )}
          <ModeSelector
          ...
          />
          {status !== "idle" && (
            <span className="rounded bg-gray-100 dark:bg-gray-700 px-1.5 py-0.5 text-[10px] text-gray-600 dark:text-gray-400">
              {status}
            </span>
          )}
        </div>
        <button
          onClick={onClose}
          className="rounded p-1 text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-gray-900 dark:hover:text-white transition-colors"
        >
          ✕
        </button>
      </div>
      ...
      {/* Messages */}
      <div ref={containerRef} className="flex-1 overflow-y-auto p-4 bg-white dark:bg-gray-900">
        {loading ? (
          ...
          <>
            ...
            {isStreaming && streamingText && (
              <div className="flex justify-start mb-3">
                <div className="max-w-[80%] rounded-lg bg-gray-50 dark:bg-gray-800 px-4 py-2 text-gray-900 dark:text-gray-100 border border-gray-200 dark:border-gray-700">
